...
class FileUtils {
  ...
  String getPathFromUri(final Context context, final Uri uri) {
    try (InputStream inputStream = context.getContentResolver().openInputStream(uri)) {
      ...
      // Jaring pengaman terakhir: sebagian device/OS (mis. Waydroid) melempar
      // NullPointerException dari dalam `ContentResolver#openInputStream()` saat
      // provider gagal menyajikan berkas. Sebelumnya exception ini tidak tertangkap
      // dan mematikan proses aplikasi. Kembalikan null agar pemanggil melaporkan
      // `missing_valid_image_uri` (error biasa) alih-alih crash.
      Log.w("FileUtils", "Failed to open input stream for " + uri + ": " + e);
      return null;
    }
  }
